...
open class RichEditorToolbar: UIView {
    ...
    open var barTintColor: UIColor? {
        get {
            return backgroundToolbar.barTintColor
        }
        set {
            backgroundToolbar.barTintColor = newValue
        }
    }
    ...
    fileprivate func updateToolbar() {
        ...
        for option in options {
            if let image = option.image {
                let button = RichBarButtonItem(image: image) { [weak self] in
                    option.action(self)
                }
                buttons.append(button)
            } else {
                let title = option.title
                let button = RichBarButtonItem(title: title) { [weak self] in
                    option.action(self)
                }
                buttons.append(button)
            }
